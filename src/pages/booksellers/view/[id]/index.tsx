...
import { getBooksellerById } from 'apiSdk/booksellers';
...
function BooksellerViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<BooksellerInterface>(
    () => (id ? `/booksellers/${id}` : null),
    () =>
      getBooksellerById(id, {
        relations: ['user', 'book', 'order'],
      }),
  );
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Bookseller Detail View
          </Text>
          {hasAccess('bookseller', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/booksellers/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('book', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/books/create?bookseller_id=${data?.id}`}>
              ...
              {hasAccess('order', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/orders/create?bookseller_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
