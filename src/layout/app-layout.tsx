...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<BooksellerInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/booksellers` : null),
    () => getBooksellers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Bookseller',
      path: isTenantUser ? `/booksellers/view/${data?.[0]?.id}` : '/booksellers',
      entity: 'bookseller',
